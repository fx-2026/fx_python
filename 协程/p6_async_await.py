...
#async 函数被调用用后会创建一个corountine
#这时候该协程并不会立即运行
#需要通过 ensure_future 或create_task方法生成task 后才会被调度执行
...
async def main():
    ...
    #asyncio.create_task()封装成task,函数用来并发运行作为 asyncio 任务的多个协程
...
async def corountine_child2_demo():
    await asyncio.sleep(5)
    return 2

tasks = [
    coroutine_child1_demo(),
    corountine_child2_demo()
]

if __name__ =="__main__":
    start = time.time()#开始时间

    ioloop = asyncio.run(tasks)
#     coroutine = asyncio.wait(tasks)
#     #Future 是一种特殊的 底层级 可等待对象，表示一个异步操作的最终结果
